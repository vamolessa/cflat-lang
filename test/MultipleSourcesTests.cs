...
public sealed class MultipleSourcesTests
{
	...
	public void CompileMultipleSourcesTests(string source0, string source1)
	{
		...
		if (errors.count > 0)
			throw new CompileErrorException(CompilerHelper.FormatError(source0, errors, 1, 1));
		errors = cflat.CompileSource("source1", source1, TestHelper.CompilerMode);
		if (errors.count > 0)
			throw new CompileErrorException(CompilerHelper.FormatError(source1, errors, 1, 1));
