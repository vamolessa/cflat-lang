...
public sealed class SetStatementTests
{
	...
	[InlineData("fn getA(a:[mut int]):[mut int]{a} fn f():int{let mut a=[0,1] set getA(mut a)[0]=9 a[0]}")]
	[InlineData("struct T{x:int} struct S{y:[mut T]} fn getS(ts:[mut T]):S{S{y=ts}} fn f():int{let mut ts=[T{x=0},1] set getS(mut ts).y[0].x=9 ts[0].x}")]
