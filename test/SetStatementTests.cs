...
public sealed class SetStatementTests
{
	...
	[InlineData("fn getA(a:[int]):[int]{a} fn f():int{let a=[0:1] set getA(a)[0]=9 a[0]}")]
	[InlineData("struct T{x:int} struct S{y:[T]} fn getS(ts:[T]):S{S{y=ts}} fn f():int{let ts=[T{x=0}:1] set getS(ts).y[0].x=9 ts[0].x}")]
